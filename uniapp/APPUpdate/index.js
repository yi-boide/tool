...
export const getServerNo = function(version,isPrompt = false, callback) {
	 ...
	 */
		 ...
		 */
		$http.post('version/index',{type:platform,version:version,'source':$http.globalData.source}, 1).then(res=>{
				// console.log(res);
				if (res && res.url) {
...
export const getDownload = function(data) {
	...
	downloadPopup(popupData, function(res) {
		dtask = plus.downloader.createDownload(data.url, {
			...
			if (status == 200) {
				...
				plus.runtime.install(download.filename, {}, function() {
					...
					res.cancel();
					plus.nativeUI.alert('Setup file failed[' + e.code + ']：' + e.message);
				});
				...
				case 3:
					...
					if(progress - lastProgressValue >= 2){
						...
						res.change({
							...
							progressTip: 'Downloading ' + progress + '%',
							progress: true
						});
					}
...
function downloadPopupDrawing(data){
	...
	// 弹窗容器高度
	let popupViewHeight = viewContentPadding * 1 + 60;
	let progressTip = data.progressTip || 'Ready to download...';
	let contentText = data.contentText || 'We are updating for you';
	let elementList = [
		{
			tag: 'rect', //背景色
			color: '#FFFFFF',
		...
		elementList = elementList.concat([
			{
				...
				position: {
					top: viewContentPadding * 2 + 20 + 'px',
					height: '30px'
				}
			},
			{
				...
				position:{
					top: viewContentPadding * 2 + 60 + 'px',
					left: viewContentPadding + 'px',
					width: viewContentWidth + 'px',
					height: '8px'
				}
		...
		change: function(res) {
			...
			if(res.progressValue){
				...
				progressElement.push({
					...
					position:{
						top: popupViewData.viewContentPadding * 2 + 60 + 'px',
			...
			if(res.progressTip){
				...
				progressElement.push({
					...
					position: {
						top: popupViewData.viewContentPadding * 2 + 20 + 'px',
						height: '30px'
					}
				});
			}
...
export default function(isPrompt = false,http) {
	$http = http;
