    ...
    <view v-if="visible || inline"
      ...
      <view v-if="showHeader"
        ...
        <view class="lb-picker-header-actions">
          ...
          <view class="lb-picker-action lb-picker-center"
            ...
            <slot name="action-center"></slot>
					...
					<view v-else class="lb-picker-action lb-picker-center">
						<text class="fz-32 fb">{{pickerTitle}}</text>
					</view>
        ...
        <slot name="header-bottom"></slot>
				<view v-if="showInput" class="header-input">
					<input class="header-input-text" type="text" :value="filterValue" placeholder="Please input search keyword" @input="filterInput"/>
				</view>
      </view>
...
export default {
  ...
  computed: {
    pickerContentHeight () {
      return this.columnHeight * this.columnNum + 'px'
    },
    isEmpty () {
      ...
      return false
		...
		getList () {
			...
			let newList = []
			oldList.map(o => {
				if (o.label.toLowerCase().indexOf(this.filterValue.toLowerCase()) != -1) {
					newList.push(o)
				}
			})
			return !this.filterValue ? oldList : newList
		}
  },
  methods: {
		filterInput(e) {
